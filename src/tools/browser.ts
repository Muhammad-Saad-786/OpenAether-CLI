...
async function withPage<T>(
  ...
  const context = await (
    await getBrowser()
  ).newContext({ viewport, deviceScaleFactor: 1 });
...
export class BrowserTool implements Tool<Input> {
  ...
  parameters = {
    ...
    properties: {
      action: {
        type: "string",
        enum: ["inspect", "screenshot", "accessibility", "visual_diff"],
      },
      ...
      baseline: {
        type: "string",
        description: "Workspace-relative baseline screenshot path.",
      },
      fullPage: {
        type: "boolean",
      ...
      height: {
        type: "number",
        description: "Viewport height, defaults to 900.",
      },
    },
    required: ["action", "url"],
  };
  ...
  async execute(input: Input): Promise<ToolResult> {
    try {
      ...
      if (input.action === "accessibility") {
            ...
            const issues = await page.evaluate(() => {
              ...
              document.querySelectorAll("img").forEach((img) => {
                if (!img.getAttribute("alt"))
                  findings.push(
                    `Image missing alt: ${img.getAttribute("src") ?? "unknown"}`,
                  );
              });
              document
                ...
                .forEach((el) => {
                  ...
                  if (!labelled)
                    findings.push(
                      `${el.tagName.toLowerCase()} has no accessible label`,
                    );
                });
              if (!document.querySelector("main, [role=main]"))
                findings.push("Page has no main landmark");
              if (!document.querySelector("h1"))
                findings.push("Page has no h1 heading");
              return findings;
            });
        ...
        return {
          ...
          summary: result.passed
            ...
            : `${result.issues.length} accessibility issue(s) found`,
          data: result,
      ...
      if (input.action === "visual_diff") {
        if (!input.baseline)
          return fail("baseline is required for visual_diff");
        ...
        return {
          ...
          summary: same
            ...
            : `Visual regression detected; screenshot saved to ${output}`,
          data: {
            output,
          ...
          error: same ? undefined : "Screenshot bytes differ from baseline",
        };
      }
      return {
        ok: true,
        toolName: this.name,
        toolCallId: "",
        summary: `Screenshot saved to ${output}`,
        data: { output, sha256: currentHash },
      };
    } catch (error) {
      return fail(
